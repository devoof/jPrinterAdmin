...
/**
 * The
 * <code>AbstractTransportMapping</code> provides an abstract implementation for
 * the message dispatcher list and the maximum inbound message size.
 *
 * @author Frank Fock
 * @version 2.0
 */
public abstract class AbstractTransportMapping<A extends Address>
    ...
    /**
     * Returns
     * <code>true</code> if asynchronous (multi-threaded) message processing may
     * be implemented. The default is
     * <code>true</code>.
     *
     ...
     *
     * @param asyncMsgProcessingSupported if <code>false</code> the
     * {@link MessageDispatcher#processMessage(org.snmp4j.TransportMapping, org.snmp4j.smi.Address, java.nio.ByteBuffer, org.snmp4j.TransportStateReference)}
     * method must not return before the message has been entirely processed,
     * because the incoming message buffer is not copied before the message is
     * being processed. If <code>true</code> the message buffer is copied for
     * each call, so that the message processing can be implemented
     * asynchronously.
     */
