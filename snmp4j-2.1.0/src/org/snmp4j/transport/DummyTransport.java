...
/**
 * The
 * <code>DummyTransport</code> is a test TransportMapping for Command Generators
 * which actually does not sent messages over the network. Instead it provides
 * the message transparently as incoming message over the
 * {@link DummyTransportResponder} on a virtual listen address, regardless to
 * which outbound address the message was sent. The messages are returned even
 * if the
 * <code>listenAddress</code> is left
 * <code>null</code>.
 *
 * @author Frank Fock
 * @since 2.0
