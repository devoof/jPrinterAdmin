...
/**
 * The
 * <code>TransportIpAddress</code> is the abstract base class for all transport
 * addresses on top of IP network addresses.
 *
 * @author Frank Fock
 * @version 1.5
 */
public abstract class TransportIpAddress extends IpAddress {
     ...
     *
     * @param transportAddress an OctetString containing the IP address bytes
     * and the two port bytes in network byte order.
     ...
     *
     * @return a byte array with IP address bytes and two additional bytes
     * containing the port in network byte order. If the address is a zoned
     * (scoped) IP address, four additional bytes with the scope ID are returned
     * between address and port bytes.
     * @since 1.5
     */
