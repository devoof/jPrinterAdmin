...
/**
 * The
 * <code>UsmUser</code> class represents USM user providing information to
 * secure SNMPv3 message exchange. A user is characterized by its security name
 * and optionally by a authentication protocol and passphrase as well as a
...
public class UsmUser implements User, Comparable, Cloneable {
     ...
     * Creates a USM user.
     *
     * @param securityName the security name of the user (typically the user
     * name).
     * @param authenticationProtocol the authentication protcol ID to be
     * associated with this user. If set to <code>null</code>, this user only
     * supports unauthenticated messages.
     * @param authenticationPassphrase the authentication passphrase. If      * not <code>null</code>, <code>authenticationProtocol</code> must
     * also be not <code>null</code>. RFC3414 §11.2 requires passphrases to have
     * a minimum length of 8 bytes. If the length      * of <code>authenticationPassphrase</code> is less than 8 bytes      * an <code>IllegalArgumentException</code> is thrown.
     * @param privacyProtocol the privacy protcol ID to be associated with this
     * user. If set to <code>null</code>, this user only supports unencrypted
     * messages.
     * @param privacyPassphrase the privacy passphrase. If      * not <code>null</code>, <code>privacyProtocol</code> must also be
     * not <code>null</code>. RFC3414 §11.2 requires passphrases to have a
     * minimum length of 8 bytes. If the length      * of <code>authenticationPassphrase</code> is less than 8 bytes      * an <code>IllegalArgumentException</code> is thrown.
     */
     ...
     * not <code>null</code>. RFC3414 §11.2 requires passphrases to have a
     * minimum length of 8 bytes. If the length      * of <code>authenticationPassphrase</code> is less than 8 bytes      * an <code>IllegalArgumentException</code> is thrown.
     * @param localizationEngineID if not <code>null</code>, the
     * localizationEngineID specifies the engine ID for which the supplied
     * passphrases are already localized. Such an USM user can only be used with
     * the target whose engine ID equals localizationEngineID.
     */
    public UsmUser(OctetString securityName,
            OID authenticationProtocol,
            OctetString authenticationPassphrase,
    ...
    /**
     * Returns the localization engine ID for which this USM user has been
     * already localized.
     *
     * @return <code>null</code> if this USM user is not localized or the SNMP
     * engine ID of the target for which this user has been localized.
     * @since 1.6
     */
     ...
     * or not (
     * <code>true</code> is returned in that case).
     *
