...
/**
 * The
 * <code>MPv3</code> is the message processing model for SNMPv3.
 *
 * @author Frank Fock
 * @version 1.9.2
 */
public class MPv3
    ...
    /**
     * Local engine ID constant for context engineID discovery as defined by RFC
     * 5343.
     */
     ...
     *
     * @param localEngineID the local engine ID. Its length must be >= 5 and <=
     * 32.
     */
    ...
    /**
     * Creates a MPv3 with a supplied local engine ID and {@link PDUFactory} for
     * incoming messages.
     *
     * @param localEngineID the local engine ID. Its length must be >= 5 and <=
     * 32. @
     * param incomingPDUFactory a {@link PDUFactory}. If <code>null</code> the
     * default factory will be used which creates {@link ScopedPDU} instances.
     * @since 1.9.1
     */
    ...
    /**
     * This is a convenience constructor which can be used to create a MPv3
     * which is bound to a specific USM instance. A dedicated USM instance per
     * MPv3 is necessary if multiple {@link Snmp} instances are used within a
     * VM.
     *
     * @param usm an USM instance.
     * @since 1.10
     */
    public MPv3(USM usm) {
     ...
     * param incomingPDUFactory a {@link PDUFactory}. If <code>null</code> the
     * default factory will be used which creates {@link ScopedPDU} instances.
     * @param secProtocols the SecurityProtocols instance to use when looking up
     * a security protocol. To get a default instance, use
     * {@link SecurityProtocols#getInstance()}.
     * @param secModels the SecurityModels instance to use when looking up a
     * security model. If you use more than one USM instance, you need to create
     * a SecurityProtocols instance (container) for each such USM instance (and
     * MPv3 combination). To get a default instance, use
     * {@link SecurityProtocols#getInstance()}.
     * @param counterSupport The CounterSupport instance to be used to count
     * events created by this MPv3 instance. To get a default instance, use
     ...
     * @return an <code>AuthenticationProtocol</code> instance if the supplied
     * ID is supported, otherwise <code>null</code> is returned.
     */
     ...
     * @return an <code>PrivacyProtocol</code> instance if the supplied ID is
     * supported, otherwise <code>null</code> is returned.
     */
     ...
     * @return a <code>SecurityModel</code> instance if the supplied ID is
     * supported, otherwise <code>null</code> is returned.
     */
    ...
    /**
     * Adds an engine ID (other than the local engine ID) to the internal
     * storage.
     *
     * @param address the <code>Address</code> of the remote SNMP engine.
     * @param engineID the engine ID of the remote SNMP engine.
     * @return <code>true</code> if the engine ID has been      * added, <code>false</code> otherwise (if the      * supplied <code>engineID</code> equals the local one).
     */
     ...
     *
     * @param address the <code>Address</code> of the remote SNMP engine.
     * @return the engine ID of the remote SNMP engine or <code>null</code> if
     ...
     *
     * @param address the <code>Address</code> of the remote SNMP engine for
     * whose engine ID is to be removed.
     * @return the removed engine ID of the remote SNMP engine      * or <code>null</code> if there is no entry for <code>address</code>
    ...
    /**
     * The
     * <code>CacheEntry</code> class holds state reference information for the
     * MPv3 message processing model for a single message.
     *
     * @author Frank Fock
     * @version 1.0
     */
    ...
    /**
     * The
     * <code>Cache</code> stores state reference information for the MPv3.
     *
     * @author Frank Fock
     * @version 1.0
     */
    protected static class Cache {
        ...
        /**
         * Adds a
         * <code>StateReference</code> to the cache. The
         * <code>PduHandle</code> of the supplied entry will be set to
         * <code>null</code> while the entry is part of the cache, because the
         * cache uses a
         * <code>WeakHashMap</code> internally which uses the
         * <code>PduHandle</code> as key. When
         *
         * @param entry the state reference to add.
         * @return {@link SnmpConstants#SNMP_MP_DOUBLED_MESSAGE} if the entry
         * already exists and {@link SnmpConstants#SNMP_MP_OK} on success.
         */
         ...
         * @return <code>true</code> if an entry has been          * deleted, <code>false</code> otherwise.
         */
         ...
         * @return a <code>CacheEntry</code> instance with the given message ID
         * or <code>null</code> if such an entry cannot be found. If a cache
         * entry is returned, the same is removed from the cache.
         */
    ...
    /**
     * The
     * <code>HeaderData</code> represents the message header information of
     * SNMPv3 message.
     *
     * @author Frank Fock
     * @version 1.0
     ...
     *
     * @param messageDispatcher Send the message on behalf the supplied
     * MessageDispatcher instance.
     * @param pdu ScopedPDU If <code>null</code>, then contextEngineID,
     * contextName, and requestID of the report generated will be zero length
     * and zero respective. Otherwise these values are extracted from the PDU.
     ...
     * @param maxSizeResponseScopedPDU the maximum size of of the report message
     * (will be most likely ignored because a report should always fit in 484
     * bytes).
     * @param stateReference the state reference associated with the original
     * message.
     * @param payload the variable binding to include in the report message.
     * @return an SNMP MPv3 error code or 0 if the report has been send
     * successfully.
     */
     ...
     *
     * @return an enterprise ID as registered by the IANA (see
     * http://www.iana.org).
     */
     ...
     *
     * @param newEnterpriseID an enterprise ID as registered by the IANA (see
     * http://www.iana.org).
     */
     ...
     *
     * @param target the <code>target</code> parameter must be ignored.
