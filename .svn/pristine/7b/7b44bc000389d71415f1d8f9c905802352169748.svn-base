 ...
 * "connectionLifecycleInterceptors" configuration property and receive events
...
public interface ConnectionLifecycleInterceptor extends Extension {
    ...
    /**
     * Called when an application calls Connection.commit(), before the driver
     * processes its own internal logic for commit().
     *
     * Interceptors should return "true" if the driver should perform its own
     * internal logic for commit(), or "false" if not.
     *
     * @return "true" if the driver should perform its own internal logic for
    ...
    /**
     * Called when an application calls Connection.rollback(), before the driver
     * processes its own internal logic for rollback().
     *
     * Interceptors should return "true" if the driver should perform its own
     * internal logic for rollback(), or "false" if not.
     *
     * @return "true" if the driver should perform its own internal logic for
     ...
     *
     * Interceptors should return "true" if the driver should perform its own
     * internal logic for setAutoCommit(), or "false" if not.
     *
     * @return "true" if the driver should perform its own internal logic for
     ...
     *
     * Interceptors should return "true" if the driver should perform its own
     * internal logic for setCatalog(), or "false" if not.
     *
     * @return "true" if the driver should perform its own internal logic for
     * setCatalog(), or "false" if not.
     *
     * @throws SQLException if an error occurs
     */
    ...
    /**
     * Called when the driver has been told by the server that a transaction is
     * now in progress (when one has not been currently in progress).
     */
    public abstract boolean transactionBegun() throws SQLException;

    /**
     * Called when the driver has been told by the server that a transaction has
     * completed, and no transaction is currently in progress.
     */
