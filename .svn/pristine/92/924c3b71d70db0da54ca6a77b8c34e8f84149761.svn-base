...
/**
 * The
 * <code>ThreadPool</code> provides a pool of a fixed number of threads that are
 * capable to execute tasks that implement the
 * <code>Runnable</code> interface concurrently. The ThreadPool blocks when all
 * threads are busy with tasks and an additional task is added.
 *
 * @author Frank Fock
 * @version 1.6
 * @since 1.0.2
...
public class ThreadPool implements WorkerPool {
     ...
     * @param size the number of threads in this pool. This number also
     * specifies the number of concurrent tasks that can be executed with this
     * pool.
    ...
    /**
     * Executes a task on behalf of this thread pool. If all threads are
     * currently busy, this method call blocks until a thread gets idle again
     * which is when the call returns immediately.
     *
     ...
     * currently busy, this method returns
     * <code>false</code>. Otherwise the task is executed in background.
     *
     * @param task a <code>Runnable</code> to execute.
    ...
    /**
     * The
     * <code>TaskManager</code> executes tasks in a thread.
     *
     * @author Frank Fock
     * @version 1.9
     * @since 1.0.2
