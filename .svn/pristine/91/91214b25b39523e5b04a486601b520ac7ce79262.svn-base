 ...
 *
 * The Object Identifier (OID) class is the encapsulation of an SMI object
 * identifier. The SMI object is a data identifier for a data element found in a
 * Management Information Base (MIB), as defined by a MIB definition. The
 * <code>OID</code> class allows definition and manipulation of object
 * identifiers.
 *
 * @author Frank Fock
 * @version 1.10.1
...
public class OID extends AbstractVariable
    ...
    /**
     * Constructs an
     * <code>OID</code> from a dotted string. The string can contain embedded
     * strings enclosed by a single quote (') that are converted to the
     * corresponding OIO value. For example the following OID pairs are equal:
     * <pre>
    ...
    /**
     * Constructs an
     * <code>OID</code> from an array of integer values.
     *
     * @param rawOID an array of <code>int</code> values. The array is copied.
     * Later changes to <code>rawOID</code> will therefore not affect the OID's
     * value.
     */
    ...
    /**
     * Constructs an
     * <code>OID</code> from two arrays of integer values where the first
     * represents the OID prefix (i.e., the object class ID) and the second one
     * represents the OID suffix (i.e., the instance identifier).
     *
     * @param prefixOID an array of <code>int</code> values. The array is
     * copied. Later changes to <code>prefixOID</code> will therefore not affect
     * the OID's value.
     * @param suffixOID an array of <code>int</code> values which will be
     * appended to the <code>prefixOID</code> OID. The array is copied. Later
     * changes to <code>suffixOID</code> will therefore not affect the OID's
     * value.
     * @since 1.8
     */
     ...
     * Later changes to <code>rawOID</code> will therefore not affect the OID's
     * value.
     * @param offset the zero based offset into the <code>rawOID</code> that
     * points to the first sub-identifier of the new OID.
     * @param length the length of the new OID,      * where <code>offset + length</code> must be less or equal the
     * length of <code>rawOID</code>. Otherwise an
     ...
     *
     * @param mask a mask where the n-th bit corresponds to the n-th
     * sub-identifier.
    ...
    /**
     * Returns the content of the as a byte array. This method can be used to
     * convert an index value to an
     * <code>OctetString</code> or
     ...
     * (index < 0 || index >= size()).
     * @return the sub-indentifier value at <code>index</code>. NOTE: The
     * returned value may be negative if the sub-identifier value is greater
     * than <code>2^31</code>.
     */
     ...
     *
     * @param index int
     * @return the sub-indentifier value at <code>index</code> as an unsigned
     * long value.
     */
    ...
    /**
     * Checks whether this
     * <code>OID</code> can be BER encoded.
     *
     * @return <code>true</code> if size() >= 2 and size() <= 128 and if the
     * first two sub-identifiers are less than 3 and 40 respectively.
     */
     ...
     * <code>OID</code> in left-to-right direction.
     *
    ...
    /**
     * Compares the n rightmost sub-identifiers in direction right-to-left with
     * those of the given
     * <code>OID</code>.
     *
     * @param n the number of sub-identifiers to compare.
    ...
    /**
     * Returns the last sub-identifier as an integer value. If this OID is empty
     * (i.e. has no sub-identifiers) then a
     * <code>NoSuchElementException</code> is thrown
     *
     * @return the value of the last sub-identifier of this OID as an integer
     * value. Sub-identifier values greater than 2^31-1 will be returned as
     * negative values!
     * @since 1.2
     */
     ...
     * <code>NoSuchElementException</code> is thrown
     *
     * @return the value of the last sub-identifier of this OID as an unsigned
     * long.
     * @since 1.2
     */
     ...
     * <code>OID</code> and returns it.
     *
     * @return the last sub-identifier or -1 if there is no sub-identifier left
     * in this <code>OID</code>.
     */
     ...
     *
     * @param n the number of subidentifiers to remove. If <code>n</code> is
     * zero or negative then this OID will not be changed. If <code>n</code> is
     * greater than {@link #size()} all subidentifiers will be removed from this
     * OID.
     */
     ...
     *
     * @return a copy of this OID with <code>n-1</code> sub-identifiers where
     * <code>n</code> is the size of this OID and greater than zero, otherwise a
     * zero length OID is returned.
     * @since 1.11
     */
    public OID trim() {
     ...
     *
     * @return if this OID ends on 0, then a {@link #MAX_OID_LEN} sub-identifier
     * OID is returned where each sub-ID for index greater or equal to
     * {@link #size()} is set to {@link #MAX_SUBID_VALUE}.
     * @since 1.7
     */
     ...
     *
     * @return OID the next OID on the same or upper level or a clone of this
     * OID, if it has a zero length or is 2^32-1.
     * @since 1.7
     */
