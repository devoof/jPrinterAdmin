...
/**
 * The
 * <code>MessageDispatcher</code> interface defines common services of instances
 * that process incoming SNMP messages and dispatch them to interested
 * {@link CommandResponder} instances. It also provides a service to send out
 * outgoing SNMP messages.
 * <p>
 * A
 * <code>MessageDispatcher</code> needs at least one {@link TransportMapping}
...
public interface MessageDispatcher extends TransportListener {
     ...
     * support a specific SNMP protocol version, the message dispatcher needs a
     * message processing model to process messages before they can be
     * dispatched.
     *
    ...
    /**
     * Removes a previously added {@link MessageProcessingModel} from the
     * dispatcher.
     *
     * @param model a <code>MessageProcessingModel</code> instance.
     */
     ...
     *
     * @param transport a <code>TransportMapping</code> instance.
     * @return the <code>TransportMapping</code> instance supplied if it could
     * be successfully removed, <code>null</code> otherwise.
     */
    TransportMapping removeTransportMapping(TransportMapping transport);

    /**
     * Gets the
     * <code>Collection</code> of transport mappings in this message dispatcher.
     *
     ...
     * uses the {@link CommandResponderEvent#setProcessed(boolean processed)} to
     * set the processed status of the event to
     * <code>true</code>.
     *
    ...
    /**
     * Removes a previously added {@link CommandResponder} instance from the
     * message dispatcher.
     *
     * @param listener a <code>CommandResponder</code> instance.
     */
     ...
     *
     * @param transportMapping the <code>TransportMapping</code> to be used to
     * send the PDU. If <code>transportMapping</code> is <code>null</code> the
     * message dispatcher will determine the appropriate transport mapping for
     * the given transport address.
     * @param target the target which identifies, transport address, message
     * processing model, security model, security name and level.
     * @param pdu the SNMP Protocol Data Unit
     * @param expectResponse <code>true</code> if a response is expected and a
     * state reference should be saved (if needed for the supplied message
     * processing model).
     ...
     * <code>PduHandle</code> just <em>before<em> the request is sent through
     * the the callback interface.
     *
     * @param transportMapping the <code>TransportMapping</code> to be used to
     * send the PDU. If <code>transportMapping</code> is <code>null</code> the
     * message dispatcher will determine the appropriate transport mapping for
    ...
    /**
     * Sends a PDU to the supplied transport address. This method behaves like a
     * call to {@link #sendPdu(TransportMapping transportMapping, Target target,
     * PDU pdu, boolean expectResponse)} with
     * <code>transportMapping</code> set to
     * <code>null</code>.
     *
     * @param target the target which identifies, transport address, message
     * processing model, security model, security name and level.
     ...
     * @throws MessageException if message processing fails with a fatal error.
     * @return an MP error status or {@link SnmpConstants#SNMP_MP_OK} if the
     ...
     * according to the message's content, the message processing models, and
     * the command responder available to the dispatcher.
     *
     ...
     * @param pduHandle the <code>PduHandle</code> that identifies a confirmed
     * class message.
