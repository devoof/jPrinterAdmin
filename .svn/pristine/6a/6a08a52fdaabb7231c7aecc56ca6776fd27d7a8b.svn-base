...
/**
 * The
 * <code>CommandResponderEvent</code> is fired by the
 * <code>MessageDispatcher</code> to listeners that potentially can process the
 * included request, report, or trap/notification.
 *
...
public class CommandResponderEvent extends EventObject {
    ...
    /**
     * Constructs an event for processing an incoming request or notification
     * PDU.
     *
     * @param messageDispatcher the source of the event. May be used to send
     * response PDUs.
     * @param transportMapping the <code>TransportMapping</code> which received
     * the PDU.
     ...
     * @param maxSizeResponseScopedPDU the maximum size of a possible response
     * PDU.
     * @param stateReference needed for responding a request, will      * be <code>null</code> for notifications.
     */
     ...
     * <code>CommandResponderEvent</code> but the source of the event is set to
     * the supplied source.
     *
    ...
    /**
     * Gets the message dispatcher instance that received the command (request
     * PDU) or unconfirmed PDU like a report, trap, or notification..
     *
     * @return the <code>MessageDispatcher</code> instance that received the
     * command.
     */
     ...
     *
     * @param processed If set to <code>true</code>, the dispatcher stops
     * dispatching this event to other event listeners, because it has been
     ...
     *
     * @return <code>true</code> if this event has been      * processed, <code>false</code> otherwise.
     */
     ...
     *
     * @return a {@link TransportStateReference} instance if the transport
     * and/or the security model supports it or <code>null</code> otherwise.
     * @since 2.0
     */
     ...
     *
     * @param tmStateReference the transport model (mapping) state information
     * associated with this command responder event.
     */
