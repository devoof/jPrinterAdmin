...
/**
 * The
 * <code>SNMP4JSettings</code> class implements a central configuration class of
 * the SNMP4J framework. As a rule of thumb, changes to the default
...
public final class SNMP4JSettings {
     ...
     * through property files defined via System properties. If set to
     * <code>false</code> all classes SNMP4J is aware of will be used hard coded
     * which speeds up initialization and is required to use SNMP4J in a secure
     * environment where System properties are not available (i.e. in an
     * unsigned applet).
     *
     * @since 1.2.2
     */
     ...
     * boundaries of API controlled threads. In SNMP4J such a thread runs in
     * each {@link TransportMapping} and in each {@link Snmp} session object. To
     * ensure robust runtime behavior, unexpected runtime exceptions are caught
     * and logged. If you need to localize and debug such exceptions then set
     * this value to
     * <code>true</code>.
     *
     * @since 1.8.1
    ...
    /**
     * By default SNMP4J uses {@link Thread} instances to run concurrent tasks.
     * For environments with restricted thread management like Java EE
     * application servers, a custom thread factory can be used.
     *
     * @since 1.9
     */
    private static ThreadFactory threadFactory = new DefaultThreadFactory();
    /**
     * By default SNMP4J uses {@link java.util.Timer} instances to run timed
     * tasks. For environments with restricted thread management like Java EE
     * application servers, a custom timer factory can be used.
     *
     * @since 1.9
     */
     ...
     * if that Thread has been exclusively used for that task). The default
     * value is 60 seconds (1 min.).
     */
     ...
     *
     * @param enable if <code>true</code> activates extensibility or      * if <code>false</code> disables it. In the latter case, SNMP4J's
     ...
     * disabled. In the latter case, SNMP4J's default configuration will be used
     ...
     * @param forwardExceptions <code>true</code> runtime exceptions are thrown
     * on thread boundaries controlled by SNMP4J and related APIs. Default      * is <code>false</code>.
     * @since 1.8.1
     */
    ...
    /**
     * Indicates whether runtime exceptions should be thrown on thread
     * boundaries controlled by SNMP4J and related APIs.
     *
     * @return <code>true</code> runtime exceptions are thrown on thread
     * boundaries controlled by SNMP4J and related APIs. Default      * is <code>false</code>.
     * @since 1.8.1
     */
     ...
     *
     * @param newThreadFactory a ThreadFactory (must not be <code>null</code>).
     * @since 1.9
     */
     ...
     *
     * @param newTimerFactory a TimerFactory (must not be <code>null</code>).
     * @since 1.9
     */
     ...
     *
     * @param newOidTextFormat the new <code>OIDTextFormat</code> (must not      * be <code>null</code>).
     * @since 1.10
     */
     ...
     *
     * @param newVariableTextFormat the new <code>VariableTextFormat</code>
     * (must not be <code>null</code>).
     * @since 1.10
     */
    public final static void setVariableTextFormat(VariableTextFormat newVariableTextFormat) {
     ...
     * is set.
     *
     * @param millis the maximum time in milli-seconds to wait for a Thread to
     * join if no explicit timeout has been set.
     * @since 1.10.2
     */
