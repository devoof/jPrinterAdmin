...
/**
 * The
 * <code>Snmp</code> class is the core of SNMP4J. It provides functions to send
 * and receive SNMP PDUs. All SNMP PDU types can be send. Confirmed PDUs can be
 * sent synchronously and asynchronously.
 * <p>
 * The
 * <code>Snmp</code> class is transport protocol independent. Support for a
 * specific {@link TransportMapping} instance is added by calling the
 * {@link #addTransportMapping(TransportMapping transportMapping)} method or
 * creating a
 * <code>Snmp</code> instance by using the non-default constructor with the
 * corresponding transport mapping. Transport mappings are used for incoming and
 * outgoing messages.
 * <p>
 * To setup a default SNMP session for UDP transport and with SNMPv3 support the
 ...
 * </pre>
 * <p>
 * How a synchronous SNMPv3 message with authentication and privacy is then sent
 * illustrates the following code snippet:
 * <p>
 * <pre>
 *   // add user to the USM
...
public class Snmp implements Session, CommandResponder {
    ...
    /**
     * The
     * <code>pendingRequests</code> table contains pending requests accessed
     * trough the key
     * <code>PduHandle</code>
     */
    ...
    /**
     * The
     * <code>asyncRequests</code> table contains pending requests accessed
     * trough the key userObject
     */
    ...
    private ArrayList<CommandResponder> commandResponderListeners;
    ...
    /**
     * Creates a
     * <code>Snmp</code> instance that uses a
     * <code>MessageDispatcherImpl</code> with no message processing models and
     * no security protols (by default). You will have to add those by calling
     * the appropriate methods on {@link #getMessageDispatcher()}.
     * <p>
     ...
     * <p>
     * To initialize a
     * <code>Snmp</code> instance created with this constructor follow this
     * sample code:
     * <pre>
     * Transport transport = ...;
     * Snmp snmp = new Snmp();
     ...
     * </pre>
     *
     * @param transportMapping TransportMapping the      * initial <code>TransportMapping</code>. You can add more or remove
     * the same later.
     */
    ...
    /**
     * Creates a
     * <code>Snmp</code> instance by supplying a
     * <code>
     * MessageDispatcher</code> and a
     * <code>TransportMapping</code>.
     * <p>
     * As of version 1.1, the supplied message dispatcher is not altered in
     * terms of adding any message processing models to it. This has to be done
     * now outside the Snmp class.
     * <p>
     * To initialize a
     * <code>Snmp</code> instance created with this constructor follow this
     * sample code:
     ...
     * </pre>
     *
     * @param messageDispatcher a <code>MessageDispatcher</code> instance that
     * will be used to dispatch incoming and outgoing messages.
     * @param transportMapping the initial <code>TransportMapping</code>, which
     * may be <code>null</code>. You can add or remove transport mappings later
     * using {@link #addTransportMapping} and {@link #removeTransportMapping}
     * respectively.
     */
     ...
     * <p>
     * The supplied message dispatcher is not altered in terms of adding any
     * message processing models to it. This has to be done now outside the Snmp
     * class.
     * </p>
     * <p>
    ...
    /**
     * Adds a
     * <code>TransportMapping</code> to this SNMP session.
     *
    ...
    /**
     * Removes the specified transport mapping from this SNMP session. If the
     * transport mapping is not currently part of this SNMP session, this method
     * will have no effect.
     *
     ...
     * <code>CommandResponder</code> with the internal
     * <code>NotificationDispatcher</code>.
     *
     * @param listenAddress the <code>Address</code> denoting the transport
     * end-point (interface and port) to listen for incoming notifications.
     * @param listener the <code>CommandResponder</code> instance that should
     * handle the received notifications.
     * @return <code>true</code> if registration was successful      * and <code>false</code> if, for example, the transport mapping for
    ...
    /**
     * Gets the next unique request ID. The returned ID is unique across the
     * last 2^31-1 IDs generated by this message dispatcher.
     *
     * @return an integer value in the range 1..2^31-1. The returned ID can be
     * used to map responses to requests send through this message dispatcher.
     * @since 1.1
     ...
     * <p>
     * If there are any pending requests, the {@link ResponseListener}
     * associated with the pending requests, will be called with a
     * <code>null</code> response and a {@link InterruptedException} in the
     * error member of the {@link ResponseEvent} returned.
     * <p>
     * After a
     * <code>Session</code> has been closed it must not be used anymore.
     *
     ...
     * target.
     *
     * @param pdu a <code>PDU</code> instance. For SNMPv3 messages, the supplied
     * PDU instance has to be a <code>ScopedPDU</code> instance.
     * @param target the Target instance representing the target SNMP engine
     ...
     * response <code>PDU</code> call {@link ResponseEvent#getResponse()}. If
     * the request timed out, that method will return <code>null</code>.
     ...
     * <code>PDU</code> to the given target. The response is then returned by
     ...
     * @param target the Target instance representing the target SNMP engine
     * where to send the <code>pdu</code>.
     * @param userHandle an user defined handle that is returned when the
     * request is returned via the <code>listener</code> object.
     * @param listener a <code>ResponseListener</code> instance that is called
     * when <code>pdu</code> is a confirmed PDU and the request is either
     ...
     * response <code>PDU</code> call {@link ResponseEvent#getResponse()}. If
     * the request timed out, that method will return <code>null</code>.
     * @throws IOException if the inform request could not be send to the
     * specified target.
     * @since 1.1
     */
     ...
     * @param target the Target instance representing the target SNMP engine
     * where to send the <code>pdu</code>. The selected SNMP protocol version
     * for the target must be {@link SnmpConstants#version1}.
     ...
     * @param target the Target instance representing the target SNMP engine
     * where to send the <code>pdu</code>. The selected SNMP protocol version
     * for the target must be {@link SnmpConstants#version2c} or
    ...
    /**
     * Sends a
     * <code>PDU</code> to the given target and if the
     * <code>PDU</code> is a confirmed request, then the received response is
     * returned synchronously.
     *
     * @param pdu a <code>PDU</code> instance. When sending a SNMPv1 trap PDU,
     * the supplied PDU instance must be a <code>PDUv1</code>. For all types of
     * SNMPv3 messages, the supplied PDU instance has to be a
     * <code>ScopedPDU</code> instance.
     * @param target the Target instance representing the target SNMP engine
     * where to send the <code>pdu</code>.
     * @param transport specifies the <code>TransportMapping</code> to be used
     * when sending the PDU. If <code>transport</code> is <code>null</code>, the
     * associated message dispatcher will try to determine the transport mapping
     * by the <code>target</code>'s address.
     * @return the received response encapsulated in      * a <code>ResponseEvent</code> instance. To obtain the received
     * response <code>PDU</code> call {@link ResponseEvent#getResponse()}. If
     * the request timed out, that method will return <code>null</code>. If the
     * sent <code>pdu</code> is an unconfirmed PDU (notification, response, or
     * report), then <code>null</code> will be returned.
     ...
     * @param target a <code>Target</code> instance denoting the target SNMP
     * entity.
     * @param transport the (optional) transport mapping to be used to send the
     * request. If <code>transport</code> is <code>null</code> a suitable
     * transport mapping is determined from the <code>target</code> address.
     * @param pduHandleCallback callback for newly created PDU handles before
     * the request is sent out.
     * @throws IOException if the transport fails to send the PDU or the if the
     * message cannot be BER encoded.
     * @return PduHandle that uniquely identifies the sent PDU for further
     * reference.
     */
    protected PduHandle sendMessage(PDU pdu, Target target,
        ...
        PduHandle handle = null;
        handle = messageDispatcher.sendPdu(tm, target,
     ...
     * <code>Snmp</code> instance. This is a convenience method that sets the
     * local engine ID in the associated
     * <code>MPv3</code> and
     * <code>USM</code>.
     *
     * @param engineID a byte array containing the local engine ID. The length
     * and content has to comply with the constraints defined in the
     * SNMP-FRAMEWORK-MIB.
     * @param engineBoots the number of boots of this SNMP engine (zero based).
     * @param engineTime the number of seconds since the value of engineBoots
     * last changed.
     * @see MPv3
     * @see USM
     */
     ...
     * because SNMP4J automatically discovers authoritative engine IDs and also
     * automatically synchronize engine time values.
     * <p>
     * <em>For this method to operate succesfully, the discover engine IDs flag
     * in {@link USM} must be
     * <code>true</code> (which is the default).
     * </em>
     *
     * @param address an Address instance representing the transport address of
     * the SNMPv3 entity for which its authoritative engine ID should be
     * discovered.
     ...
     *
     * @return the <code>USM</code> instance associated with the MPv3 bound to
     * this <code>Snmp</code> instance, or <code>null</code> otherwise.
     */
    public USM getUSM() {
     ...
     *
     * @param messageProcessingModel a mesage processing model ID as defined in
     * {@link MessageProcessingModel}.
     * @return MessageProcessingModel a <code>MessageProcessingModel</code> if
     * <code>messageProcessingModel</code> has been registered with the message
     * dispatcher associated with this SNMP session.
     */
     ...
     *
     * @param event a <code>CommandResponderEvent</code> with the decoded
     * incoming PDU as dispatched to this method call by the associated message
     * dispatcher.
     */
    ...
    /**
     * Checks whether RFC5343 based context engine ID discovery is disabled or
    ...
    /**
     * Sets the RFC5343 based context engine ID discovery. The default value is
     * <code>false</code>.
     *
     * @param contextEngineIdDiscoveryDisabled <code>true</code> to disable
     * context engine ID discovery, <code>false</code> to enable context engine
     * ID discovery.
     * @since 2.0
     */
    ...
    /**
     * Removes a
     * <code>CommandResponder</code> from this SNMP session.
     *
     * @param listener a previously added <code>CommandResponder</code>
     * instance.
     */
    ...
    /**
     * Adds a
     * <code>CommandResponder</code> to this SNMP session. The command responder
     * will then be informed about incoming SNMP PDUs of any kind that are not
     * related to any outstanding requests of this SNMP session.
     *
    ...
    /**
     * Fires a
     * <code>CommandResponderEvent</code> event to inform listeners about a
     * received PDU. If a listener has marked the event as processed further
     ...
     * sends retries whenever the time specified by the
     * <code>timeout</code> parameter of the target has elapsed without a
     * response being received for the request. By specifying a different
     * timeout model this behaviour can be changed.
     *
     ...
     *
     * @param reportHandler a <code>ReportHandler</code> instance which must not
     * be <code>null</code>.
     * @since 1.6
     */
    ...
    class PendingRequest extends TimerTask implements PduHandleCallback<PDU>, Cloneable {
        ...
        /**
         * The
         * <code>nextPDU</code> field holds a PDU that has to be sent when the
         * response of the
         * <code>pdu</code> has been received. Usually, this is used for
         * (context) engine ID discovery.
         */
        private PDU nextPDU;
        ...
        /**
         * Cancels the request and clears all internal fields by setting them to
         * <code>null</code>.
         *
    ...
    /**
     * The
     * <code>NotificationDispatcher</code> dispatches traps, notifications, and
    ...
    class NotificationDispatcher implements CommandResponder {
        ...
        public synchronized void closeAll() {
            ...
            for (Iterator<TransportMapping> it = notificationListeners.values().iterator();
                    it.hasNext();) {
                TransportMapping tm = it.next();
                try {
                    tm.close();
         ...
         *
         * @param event the <code>CommandResponderEvent</code> with the INFORM
         * request.
         * @throws MessageException if the response could not be created and
         * sent.
         */
