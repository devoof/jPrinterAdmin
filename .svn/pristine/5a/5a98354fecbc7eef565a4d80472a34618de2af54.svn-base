...
public class TreeUtils extends AbstractSnmpUtility {
    ...
    /**
     * Creates a
     * <code>TreeUtils</code> instance. The created instance is thread safe as
     ...
     * that are used by this instance to retrieve MIB tree data using
     ...
     *
     * @param target a <code>Target</code> that specifies the target command
     * responder including its network transport address.
     * @param rootOID the OID that specifies the root of the sub-tree to
     * retrieve (not included).
     * @return a possibly empty List of <code>TreeEvent</code> instances where
     * each instance carries zero or more values (or an error condition) in
     * depth-first-order.
     */
    ...
    /**
     * Walks a subtree with GETNEXT (SNMPv1) or GETBULK (SNMP2c, SNMPv3)
     * operations from the specified target asynchronously.
     *
     * @param target a <code>Target</code> that specifies the target command
     * responder including its network transport address.
     * @param rootOIDs the OIDs which specify the subtrees to walk. Each OID
     * defines a sub-tree that is walked. The walk ends if (a) an SNMP error
     * occurs, (b) all returned variable bindings for an iteration contain an
     * exception value (i.e., {@link Null#endOfMibView}) or for each rootOIDs
     * element, the returned VariableBinding's OID has not the same prefix, (c)
     * a VariableBinding out of lexicographic order is returned.
     * @return a possibly empty List of <code>TreeEvent</code> instances where
     * each instance carries zero or <code>rootOIDs.length</code> values.
     * @since 2.1
     */
     ...
     * @param rootOID the OID that specifies the root of the sub-tree to
     * retrieve (not included).
     * @param userObject an optional user object that will be transparently
     * handed over to the supplied <code>TreeListener</code>.
     * @param listener the <code>TreeListener</code> that processes the
     * {@link TreeEvent}s generated by this method. Each event object may carry
     * zero or more object instances from the sub-tree in depth-first-order.
     */
     ...
     * @param listener the <code>TreeListener</code> that processes the
     * {@link TreeEvent}s generated by this method. Each event object may carry
     * zero or more object instances from the sub-tree in depth-first-order if
     * rootOIDs has a single element. If it has more than one element, then each
     ...
     *
     * @param maxRepetitions the maximum repetitions used for GETBULK requests.
     * For SNMPv1 this values has no effect (it is then implicitly one).
     */
     ...
     *
     * @param ignoreLexicographicOrder <code>true</code> to ignore lexicographic
     * order errors, <code>false</code> otherwise (default).
     * @since 1.10.1
     */
