...
/**
 * The
 * <code>TableUtils</code> class provides utility functions to retrieve SNMP
 * tabular data.
 *
 * @author Frank Fock
 * @version 1.6e
...
public class TableUtils extends AbstractSnmpUtility {
    ...
    /**
     * Creates a
     * <code>TableUtils</code> instance. The created instance is thread safe as
     * long as the supplied
     * <code>Session</code> and
     * <code>PDUFactory</code> are thread safe.
     *
     * @param snmpSession a SNMP <code>Session</code> instance.
     * @param pduFactory a <code>PDUFactory</code> instance that creates the PDU
     * that are used by this instance to retrieve table data using
    ...
    /**
     * Gets synchronously SNMP tabular data from one or more tables. The data is
     * returned row-by-row as a list of {@link TableEvent} instances. Each
     * instance represents a row (or an error condition). Besides the target
     * agent, the OIDs of the columnar objects have to be specified for which
     * instances should be retrieved. With a lower bound index and an upper
     * bound index, the result set can be narrowed to improve performance. This
     * method can be executed concurrently by multiple threads.
     *
     * @param target a <code>Target</code> instance.
     * @param columnOIDs an array of OIDs of the columnar objects whose
     * instances should be retrieved. The columnar objects may belong to
     * different tables. Typically they belong to tables that share a common
     * index or sub-index prefix. Note: The result of this method is not defined
     * if instance OIDs are supplied in this array!
     * @param lowerBoundIndex an optional parameter that specifies the lower
     * bound index. If not <code>null</code>, all returned rows have an index
     * greater than <code>lowerBoundIndex</code>.
     * @param upperBoundIndex an optional parameter that specifies the upper
     * bound index. If not <code>null</code>, all returned rows have an index
     * less or equal than <code>upperBoundIndex</code>.
     * @return a <code>List</code> of {@link TableEvent} instances. Each
     * instance represents successfully retrieved row or an error condition.
     * Error conditions (any status other than {@link TableEvent#STATUS_OK}) may
     * only appear at the last element of the list.
     */
     ...
     * asynchronously row-by-row through a supplied callback. Besides the target
     * agent, the OIDs of the columnar objects have to be specified for which
     * instances should be retrieved. With a lower bound index and an upper
     * bound index, the result set can be narrowed to improve performance.
     * <p>
     * This method may call the {@link TableListener#finished} method before it
     * returns. If you want to synchronize the main thread with the finishing of
     * the table retrieval, follow this pattern:
     * <pre>
     *      synchronized (this) {
     ...
     * index or sub-index prefix. Note: The result of this method is not defined
     * if instance OIDs are supplied in this array!
     * @param listener a <code>TableListener</code> that is called with
     * {@link TableEvent} objects when an error occured, new rows have been
     * retrieved, or when the table has been retrieved completely.
     * @param userObject an user object that is transparently supplied to the
     * above call back.
     * @param lowerBoundIndex an optional parameter that specifies the lower
     * bound index. If not <code>null</code>, all returned rows have an index
     * greater than <code>lowerBoundIndex</code>.
     ...
     * bound index, the result set can be narrowed to improve performance.
     * <p>
     * This implementation must not be used with sparese tables, because it is
     * optimized for dense tables and will not return correct results for sparse
     * tables.
     * </p>
     *
     * @param target a <code>Target</code> instance.
     ...
     * @param upperBoundIndex an optional parameter that specifies the upper
     * bound index. If not <code>null</code>, all returned rows have an index
     * less or equal than <code>lowerBoundIndex</code>.
     * @since 1.5
     */
     ...
     * request.
     *
     * @return an integer greater than zero that specifies the maximum number of
     * rows to retrieve per SNMP GETBULK operation.
     */
     ...
     *
     * @param numberOfRowsPerChunk an integer greater than zero that specifies
     * the maximum number of rows to retrieve per SNMP GETBULK operation.
     */
    ...
    /**
     * Gets the maximum number of columns that will be retrieved per SNMP
     * GETNEXT or GETBULK request.
     *
     * @return an integer greater than zero that specifies the maximum columns
     * of rows to retrieve per SNMP GETNEXT or GETBULK operation.
     */
    ...
    /**
     * Sets the maximum number of columns that will be retrieved per SNMP
     * GETNEXT or GETBULK request. The default is 10.
     *
     * @param numberOfColumnsPerChunk an integer greater than zero that
     * specifies the maximum columns of rows to retrieve per SNMP GETNEXT or
     * GETBULK operation.
     */
    ...
    /**
     * The
     * <code>DenseTableRequest</code> extends TableRequest to implement a faster
     * table retrieval than the original. Caution: This version does not
     * correctly retrieve sparse tables!
     *
     * @author Frank Fock
     * @since 1.5
     */
                ...
                for (int r = 0; r < rows; r++) {
                    ...
                    if (row != null) {
                        if (!listener.next(new TableEvent(this, userObject, row.getRowIndex(),
                                (VariableBinding[]) row.toArray(new VariableBinding[0])))) {
                            finished = true;
                            listener.finished(new TableEvent(this, userObject));
                            return;
                        }
     ...
     * @param rowStatusColumnOID the column OID of the RowStatus column (without
     * any instance identifier).
     * @param rowIndex the OID denoting the index of the table row to create.
     * @param values the values of columns to set in the row.      * If <code>values</code> is <code>null</code> the row is created via
     * the tripple mode row creation mechanism (RowStatus is set to
     * createAndWait). Otherwise, each variable binding has to contain the OID
     * of the columnar object ID (without any instance identifier) and its
     * value. On return, the variable bindings will be modified so that the
     * variable binding OIDs will contain the instance OIDs of the respective
     * columns (thus column OID + rowIndex).
     * @return ResponseEvent the ResponseEvent instance returned by the SNMP
     * session on response of the internally sent SET request.      * If <code>null</code>, an IO exception has occurred. Otherwise, if
     * the response PDU is <code>null</code> a timeout has occured, Otherwise,
     * check the error status for {@link SnmpConstants#SNMP_ERROR_SUCCESS} to
     * verify that the row creation was successful.
     * @since 1.6
     */
