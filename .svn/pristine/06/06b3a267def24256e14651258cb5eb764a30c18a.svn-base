...
/**
 * The
 * <code>PDUv1</class> represents SNMPv1 PDUs. The behavior of this class
 ...
 * implemented by
 * <code>PDU</code> are not supported. In contrast to its super class,
 * <code>PDUv1</code> implements the {@link PDU#V1TRAP} type.
 *
 * <p>To support this type, access methods are provided to get and set the
 * enterprise
 * <code>OID</code>, generic, specific, and timestamp of a SNMPv1 trap PDU.
 *
...
public class PDUv1 extends PDU {
    ...
    /**
     * A coldStart(0) trap signifies that the SNMP entity, supporting a
    ...
    /**
     * A warmStart(1) trap signifies that the SNMP entity, supporting a
     * notification originator application, is reinitializing itself such that
     * its configuration is unaltered.
     */
    public static final int WARMSTART = 1;
    /**
     * A linkDown(2) trap signifies that the SNMP entity, acting in an agent
     ...
     * state (but not from the notPresent state). This other state is indicated
     * by the included value of ifOperStatus.
     */
    public static final int LINKDOWN = 2;
    /**
     * A linkUp(3) trap signifies that the SNMP entity, acting in an agent role,
    ...
    /**
     * An authenticationFailure(4) trap signifies that the SNMP entity has
     ...
     * <code>ENTERPRISE_SPECIFIC</code>(6), then the enterprise specific trap ID
     * is given by the specificTrap member field.
     */
     ...
     * Copy constructor.
     *
     * @param other the <code>PDUv1</code> to copy from.
     * @since 1.9.1c
     */
     ...
     * @return <code>true</code> if the variable is SNMPv1 compatible,
     ...
     * iso(1).org(3).dod(6).internet(1).private(4).enterprises(1) node, but
     * that's not true.
     *
    ...
    /**
     * Gets the IP address of the originator system of this SNMPv1 trap. If this
     * value is 0.0.0.0 (the recommended default), then the address of the peer
     * SNMP entity should be extracted from the {@link Target} object associated
     * with this PDU.
     *
     ...
     * {@link #setSpecificTrap} must be used to set the trap ID of the
    ...
    /**
     * Gets the
     * <code>TimeTicks</code> value of the trap sender's notion of its sysUpTime
     * value when this trap has been generated.
     *
     * @return a long value.
     * @throws UnsupportedOperationException if the type of this PDU is not
     * {@link PDU#V1TRAP}.
