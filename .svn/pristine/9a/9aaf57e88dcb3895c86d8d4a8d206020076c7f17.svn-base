...
/**
 * The
 * <code>TLSTM</code> implements the Transport Layer Security Transport Mapping
 * (TLS-TM) as defined by RFC 5953 with the new IO API and
 * {@link javax.net.ssl.SSLEngine}.
 * <p>
 * It uses a single thread for processing incoming and outgoing messages. The
 * thread is started when the
 * <code>listen</code> method is called, or when an outgoing request is sent
 * using the
...
public class TLSTM extends TcpTransportMapping {
    ...
    /**
     * Creates a TLS transport mapping with the server for incoming messages
     * bind to the given address. The
     * <code>securityCallback</code> needs to be specified before
     * {@link #listen()} is called.
     *
     * @throws java.io.IOException on failure of binding a local port.
     */
     ...
     *
     * @param securityCallback a security name callback to resolve X509
     * certificates to tmSecurityNames.
     * @param serverAddress the TcpAddress instance that describes the server
     * address to listen on incoming connection requests.
     * @throws java.io.IOException if the given address cannot be bound.
     */
     ...
     * events created by this TLSTM instance. To get a default instance, use
     ...
     *
     * @param tlsProtocols an array of TLS protocol (version) names supported by
     * the SunJSSE provider. The order in the array defines which protocol is
     * tried during handshake first.
     * @since 2.0.3
     */
    ...
    /**
     * Sets the certificate alias used for client and server authentication by
     * this TLSTM. Setting this property to a value other than
     * <code>null</code> filters out any certificates which are not in the chain
     * of the given alias.
     *
     * @param localCertificateAlias a certificate alias which filters a single
     * certification chain from the <code>javax.net.ssl.keyStore</code> key
     * store to be used to authenticate this TLS transport mapping.      * If <code>null</code> no filtering appears, which could lead to
     * more than a single chain available for authentication by the peer, which
     * would violate the TLSTM standard requirements.
     */
    ...
    /**
     * Set the TLSTM trust manager factory. Using a trust manager factory other
     * than the default allows to add support for Java 1.7
     * X509ExtendedTrustManager.
     *
     * @param trustManagerFactory a X.509 trust manager factory implementing the
     * interface {@link TLSTMTrustManagerFactory}.
     * @since 2.0.3
     */
    ...
    /**
     * Listen for incoming and outgoing requests. If the
     * <code>serverEnabled</code> member is
     * <code>false</code> the server for incoming requests is not started. This
     * starts the internal server thread that processes messages.
     *
     * @throws java.net.SocketException when the transport is already listening
     ...
     * @throws java.io.IOException if the remote address cannot be closed due to
     * an IO exception.
     * @since 1.7.1
     */
     ...
     * @param tmStateReference the (optional) transport model state reference as
     * defined by RFC 5590 section 6.1.
     * @throws java.io.IOException
     */
     ...
     * may be idle before it is closed.
     *
     * @param connectionTimeout the idle timeout in milliseconds. A zero or
     * negative value will disable any timeout and connections opened by this
     * transport mapping will stay opened until they are explicitly closed.
     */
    ...
    /**
     * Sets whether a server for incoming requests should be created when the
     * transport is set into listen state. Setting this value has no effect
     * until the {@link #listen()} method is called (if the transport is already
     * listening, {@link #close()} has to be called before).
     *
     * @param serverEnabled if <code>true</code> if the transport will listens
     * for incoming requests after {@link #listen()} has been called.
     */
     ...
     * nothing.
     *
     * @param serverSocket the <code>ServerSocket</code> to apply additional
     * non-default options.
     */
...
//      sslEngineConfigurator.configure(SSLContext.getDefault(), useClientMode);
    ...
    class ServerThread implements WorkerTask {
         ...
         *
         * @param result the SSLEngine wrap/unwrap result.
         * @param entry the session to use.
         * @return <code>true</code> if processing of delegated tasks has been
...
//          entry.addRegistration(selector, SelectionKey.OP_WRITE);
    ...
    static interface SSLEngineConfigurator {
        ...
        /**
         * Configure the supplied SSLEngine for TLS. Configuration includes
         * enabled protocol(s), cipher codes, etc.
         *
         ...
         *
         * @param useClientMode <code>true</code> if the connection is
         * established in client mode.
         * @param transportStateReference the transportStateReference with
         * additional security information for the SSL connection to establish.
