...
/**
 * The
 * <code>MultiThreadedMessageDispatcher</code> class is a decorator for any
 * <code>MessageDispatcher</code> instances that processes incoming message with
 * a supplied
 * <code>ThreadPool</code>. The processing is thus parallelized on up to the
 * size of the supplied thread pool threads.
 * <p>
 ...
 * TransportStateReference tmStateReference)} to allow concurrent processing of
 * the buffer.
 *
 * @author Frank Fock
 * @version 2.0
...
public class MultiThreadedMessageDispatcher implements MessageDispatcher {
     ...
     * <code>ThreadPool</code> to concurrently process incoming messages that
     * are forwarded to the supplied decorated
     * <code>MessageDispatcher</code>.
     *
     * @param workerPool a <code>WorkerPool</code> instance (that can be
     * shared). <em>The worker pool has to be stopped externally.</em>
     * @param decoratedDispatcher the decorated <code>MessageDispatcher</code>
