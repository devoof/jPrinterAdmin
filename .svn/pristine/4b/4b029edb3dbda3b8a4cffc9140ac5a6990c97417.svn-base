...
/**
 * The
 * <code>SnmpConfigurator</code> class configures a {@link Snmp} instance with
 * settings taken from a
 * <code>Map</code> conforming to the format returned by
 * {@link ArgumentParser#parse(String[] args)}. In addition, a
 * {@link PDUFactory} and {@link Target} can be created using settings from a
 * <code>Map</code> too.
 *
 * @author Frank Fock
 * @version 1.10
...
public class SnmpConfigurator {
    ...
    public void configure(Snmp snmp, Map settings) {
        if (snmp.getUSM() != null) {
            ...
            OctetString localEngineID =
                    createOctetString((String) ArgumentParser.getValue(settings, oLocalEngineID, 0),
                    null);
    ...
    public Target getTarget(Map settings) {
        ...
        OctetString community =
                createOctetString((String) ArgumentParser.getValue(settings, oCommunity, 0),
                "public");
     ...
     *
     * @param s the string value to convert. If it starts with <code>0x</code>
     * its trailing value will be interpreted as a hex string with colon (:)
     * separator.
     * @param defaultString an optional default if <code>s</code>      * is <code>null</code>.
